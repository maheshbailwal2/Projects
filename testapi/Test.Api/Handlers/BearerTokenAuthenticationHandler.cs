// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BearerTokenAuthenticationHandler.cs" company="Media Valet Inc.">
...
using MediaValet.ServiceLibrary.Content.Enums;
using Newtonsoft.Json;

namespace MediaValet.Api.Security
{
    /// <summary>
    /// Message Handler for decoding the bearer token.
    /// </summary>
    public class BearerTokenAuthenticationHandler : DelegatingHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (tokenIpAddress == null || tokenIpAddress.Value != GetClientIp(request))
                {
                    var response = BuildResponseErrorMessage(HttpStatusCode.Unauthorized);
                    return Task.FromResult(response);
                }
        ...
        private static HttpResponseMessage BuildResponseErrorMessage(HttpStatusCode statusCode)
        {
            ...
            var authenticateHeader = new AuthenticationHeaderValue(
                "Bearer",
                "authorization_uri=\"" + "mediavaleturl" + "\"" + "," + "resource_id=" + "audience");
        ...
        private static void GetClaimObjects()
        {
            ...
            if (userClaims != null)
            {
                ...
                var orgUnit = userPermissionList.FirstOrDefault(
                 p => p.SecurableObjectType.Equals(EnumSecurableObjectType.OrgUnit));
                var repository = userPermissionList.FirstOrDefault(
                p => p.SecurableObjectType.Equals(EnumSecurableObjectType.Repository));
                ...
                var sId = JsonConvert.DeserializeObject(userClaims.Claims.Where(p => p.Type.Contains("SId")).Select(k => k.Value).FirstOrDefault());
                var email = userClaims.Claims.Where(p => p.Type.Contains("Email")).Select(k => k.Value).FirstOrDefault();

                var authenticatedUser = new AuthenticatedUser(
                    new EntityId(Guid.Parse(userId.ToString())),
                    new EmailAddress(email),
                    userPermissionList)
                                            {
                                                OrgUnitId = orgUnit != null ? orgUnit.ObjectId : Guid.Empty,
                                                SId = sId != null ? Guid.Parse(sId.ToString()) : Guid.Empty,
                                                RepositoryId = repository != null ? repository.ObjectId : Guid.Empty
                                            };
