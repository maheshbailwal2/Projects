...
public sealed class Parser
{
    ...
    public int GetLastExceptionLine
    {
        get { return GetTokenLineNumber(); }
    }
    ...
    private void ParseClassVariable(TypeDeclaration _type)
    {
        ...
        if (this.tokens[this.index].Equals(languageSetting["VariableDeclaration"]))
        {
            ...
            {
                ParsingError("expected end of statement");
            }
    ...
    private MethodDeclaration ParseMethod()
    {
        ...
        if (this.tokens[this.index++] != Scanner.ClosingParentheses)
            ParsingError("Closing Paranthisis Expected");
    ...
    private Expr ParseExpr()
    {
        if (this.index == this.tokens.Count)
        {
            ParsingError("expected expression, got EOF");
        }
        ...
        while (this.tokens[this.index] != Scanner.Semi)
        {
            ...
            {
                ParsingError("expected string literal, int literal, or variable");
            }
        }
    ...
    private MethodCall ParseMethodCall(bool IsConstructor)
    {
        ...
        if (this.tokens[this.index++] != Scanner.OpeningParentheses)
            ParsingError("Invalid Function Syntax");
    ...
    {
        return _scanner.GetTokenLineNumber(this.index);

    }

    private void ParsingError(string msg)
    {
        throw new ParserException(GetTokenLineNumber(), msg);
    }
}
...
{
    public ParserException(int lineNumber, string message):base(lineNumber,message)
    {
    }
}
